...
public class MoneyController implements Listener{
	...
	public static void setup() {
		...
		if (!useVault)
			Bukkit.getServer().getPluginManager().registerEvents(new MoneyController(), BattleCommandLimiter.getSelf());		
	}
	...
	private static void checkRegisteredPlugins(){
		...
		if (controller != null) {
			...
			if (!hasVault)
				useVault = false;
			Log.info(BattleCommandLimiter.getVersion() +" found economy plugin Register");
		}
    	if (MoneyController.economy == null){ /// We want to use vault if we can
    		...
    		if (controller != null) {
    			...
    			Log.info(BattleCommandLimiter.getVersion() +" found economy plugin Vault. [Default]");
    		}
    	}
